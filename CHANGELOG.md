...
- **`logic://tracks` returns placeholder rows on Logic Pro 12.2 + macOS 26** — `AXLogicProElements.getTrackHeaders` Strategy 3 (the AXGroup-by-description matcher) accepted only `desc.lowercased() == "track headers"` or `"트랙 헤더"`. Live Logic 12.2 on macOS 26.3 exposes the rail as `AXGroup desc="Tracks header"` — singular `"Tracks"`, lowercase `"header"` — which lowercases to `"tracks header"` and missed the existing equality by a single letter plus word-order flip, with no structural change. Result: `getTrackHeaders` returned `nil`, `StatePoller.pollTracks` cached nothing, and `ResourceHandlers.readTracks` fell through to Tier 2 placeholder synthesis (`source: "ax_live_with_file_count"`, `name: "Track \(idx + 1)"`, `placeholder: true`) even when MCU was fully registered and the mixer AX poll was healthy. Strategy 3 now accepts `track header(s)` / `tracks header(s)` plus the existing Korean string; new unit tests cover the Logic 12.2 form and pin the legacy + Korean spellings against future regressions. Mirrors the shape of the v3.4.5 Logic 12.2 mixer-matcher restoration. Verified live on macOS 26.3.1 + Logic Pro 12.2 — `source` flips to `ax_live`, real track names surface, `placeholder` is no longer set on returned rows.
