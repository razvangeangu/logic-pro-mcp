...
- **`logic_plugins` verified plugin apply-back surface (v3.6.0, additive — 9 tools total)** — a new `logic_plugins` tool exposes three commands backed by the HC v2 (`hc_schema: 2`) envelope for the verified plugin path. Every `logic_plugins.*` response carries `state` (`"A"` / `"B"` / `"C"`) and `hc_schema: 2`; the existing 8-tool surface is byte-identical.
  ...
  - **`insert_verified`** — live, readback-verified plugin insert via Logic's **Mix > Search and Add Plug-in…** menu path. Enforces the pre-insert gates (schema → mode → project path → identity [insertable allowlist: Gain / Channel EQ / Compressor] → inventory `complete:true` → slot `empty`), then drives a clean-state insert (hide plug-in windows + close any stray "위치로 이동" dialog + raise the mixer, click Search and Add via AppleScript with `enabled` polling, type the plugin name, commit the auto-highlighted first result with Return) and diffs the pre/post insert inventory. **State A only when the post-insert readback observes the requested plugin newly mounted at the requested slot** — the readback diff is the sole State A path, so a false verified insert is structurally impossible. **라이브 E2E 검증 완료 (2026-06-15, 멀티-plugin)**: `insert_verified` with `plugin: "gain"`, `"compressor"`, and `"channel eq"` all returned `verified: true` (`winning_strategy: "return_only"`); independent `get_inventory` readback confirmed the exact `plugin_id` at the mounted slot (Gain/Compressor at insert 6, Channel EQ at insert 5). Return-only is essential: the dialog auto-highlights the exact-name match, so an extra Down would select a wrong variant (e.g. "Squash Compressor") — caught regardless by the readback gate as `post_insert_plugin_mismatch`. **Slot-targeting limitation (Release 1):** Search and Add lets Logic choose the slot (not caller-controllable); when it differs from the requested `insert` the op fails closed with State C `insert_landed_at_different_slot` (reports `observed_slot`) and rolls the stray mount back (Edit > Undo, confirmed by readback). Replaces the legacy `insert_plugin` unverified path for the allowlisted stock plugins.
