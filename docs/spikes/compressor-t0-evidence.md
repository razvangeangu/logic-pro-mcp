...
> **이후 경과 (2026-06-15):** T6 `insert_verified`는 이후 "Mix > Search and Add Plug-in" 경로로 라이브 State A 검증 완료(insert 6 Gain mount). 위 `not_implemented`는 이 spike 시점(2026-06-14)의 관측이며 현재 동작은 아니다 — 현 동작은 `docs/API.md` / `docs/guides/verified-apply-back.md` 참조. 이 표는 T0 gate 충족 당시의 기록으로 보존한다.
