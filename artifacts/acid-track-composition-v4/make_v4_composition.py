#!/usr/bin/env python3
# SIZE_OK: Artifact generator keeps composition data and MIDI rendering together for reproducible fixture rebuilds.
...
def main() -> None:
    ...
    for part in parts:
        ...
        artifact_relative_path = artifact_path.relative_to(ROOT)
        ...
        output.append(
            {
                ...
                "artifact_midi_path": str(artifact_relative_path),
            }
        )
        ...
        for idx, part in enumerate(output, start=20):
            request = {
                ...
                "params": {
                    ...
                    "arguments": {
                        "command": "import_file",
                        "params": {"path": f"midi/{part['name']}.mid"},
                    },
                },
            }
    ...
    brief = [
        ...
        "- Logic package: generated locally from the MIDI fixture; not tracked because Logic embeds user-local library/bookmark paths",
        "- Verification target: 11 MIDI regions, 0 packaged audio files, no imported reference/stem audio",
        "- Import runner: `import_v4_sequential.py` copies MIDI files to the LogicProMCP managed temp directory and waits for `verified:true` after each import",
        "",
        "## Tracked Artifact Policy",
        "",
        "This directory intentionally keeps a small, reviewable MIDI-only fixture for v4 validation. The MIDI files are generated by `make_v4_composition.py`; `v4-layer-specs.json` and `v4-import-requests.jsonl` store artifact-relative `midi/...` paths so the fixture is relocatable. Do not track generated `.logicx` packages here: Logic embeds user-local library/bookmark paths in the package. If a generated package must be shared, attach it to a release or store it in Git LFS with a fresh provenance note.",
        "",
        ...
        "- Live validation generated a local Logic package and package-level `ProjectData` inspection found every expected v4 region name listed below. AX viewport readback can show fewer visible rows, so the package check is validation evidence, not a tracked source artifact.",
